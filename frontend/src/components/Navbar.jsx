...
export function Navbar({ utilisateur, user: userProp }) {
  ...
  return <nav className="fixed top-0 left-0 right-0 z-50 bg-white/80 dark:bg-library-surface/80 backdrop-blur-xl border-b border-muted transition-all duration-300 shadow-sm dark:shadow-lg adaptive-fg">
  ...
  >
                ...
                <span className="absolute top-1.5 right-1.5 w-2 h-2 bg-library-accent rounded-full border-2 border-library-surface group-hover:animate-pulse" />
              </button>
              {showNotifications && <div className="absolute right-0 mt-2 w-80 z-50">
  ...
  >
    ...
    src={user?.avatar && (user.avatar.startsWith("/") || user.avatar.startsWith("http")) ? user.avatar : void 0}
  ...
  />
              ...
              {showProfileMenu && <div className="absolute right-0 mt-2 w-56 z-50">
