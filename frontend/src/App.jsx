...
/**
 * KossiRedirect : Redirige vers l'interface Next.js Kossi (assistant IA)
 * L'interface Kossi est une application séparée sur le port 3001.
 * On utilise window.location.href pour quitter l'app React et ouvrir Kossi.
 */
function KossiRedirect() {
  const KOSSI_UI_URL = import.meta.env.VITE_KOSSI_UI_URL || "http://localhost:3000";
  useEffect(() => {
  ...
  return (
    <div className="min-h-screen bg-library-bg flex flex-col items-center justify-center gap-4">
      <div className="w-14 h-14 rounded-2xl bg-[var(--library-accent)] flex items-center justify-center shadow-elevated">
        <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M12 2a10 10 0 1 0 10 10H12V2z" /><path d="M12 2a10 10 0 0 1 10 10" /><circle cx="12" cy="12" r="3" /></svg>
      </div>
      <p className="text-[var(--library-muted)] text-sm">Ouverture de Kossi IA…</p>
...
function AppRoutes() {
  ...
  return <>
    ...
    <Routes>
      ...
      {/* Chat : redirige vers l'interface Kossi IA (app Next.js séparée sur port 3001). */}
