...
export function LandingPage() {
  ...
  const displayStats = {
    ...
    years: stats?.years ?? (/* @__PURE__ */ new Date()).getFullYear() - 1978
  };
  ...
  return <div className="min-h-screen bg-library-bg page-landing">
  ...
  >
              ...
              <div className="flex items-center gap-6 pt-2">
                <div className="flex -space-x-3">
                  {["/avarta-1.png", "/avarta-2.png", "/avarta-3.png"].map((src, i) => <img key={i} src={src} alt="" className="w-10 h-10 rounded-full border-2 border-[var(--library-surface)] object-cover" />)}
                </div>
  ...
  >
          ...
          <div className="flex gap-6 overflow-x-auto scrollbar-hide pb-4">
            {popularBooks.map((book, index) => <div
    key={book.title}
  ...
  ].map((stat) => <div key={stat.label} className="text-center p-3 surface-alt rounded-xl border border-[var(--border-color)]">
  ...
  ].map((item) => <div key={item.title} className="card">
  ...
  ].map((item) => <div key={item.day} className={`p-4 rounded-xl border ${item.open ? "surface-alt border-[var(--border-color)]" : "surface-weak border-[var(--border-color)] opacity-60"}`}>
  ...
  ].map((field) => <div key={field.label} className={field.span ? "sm:col-span-2" : ""}>
  ...
  }} className="btn-solid w-full h-12 font-bold shadow-medium hover:shadow-elevated hover:-translate-y-0.5 transition-all sheen relative overflow-hidden">
                  Envoyer le message
                </Button>
              </div>
            </form>
