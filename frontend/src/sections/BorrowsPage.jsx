...
function EmptyBorrows() {
  return <div className="empty-state py-16 surface rounded-2xl border border-[var(--border-color)]">
      <svg className="empty-state-illustration" viewBox="0 0 120 120" fill="none" xmlns="http://www.w3.org/2000/svg">
        {
    /* Livre ouvert */
  }
        <path
    d="M20 35 L60 42 L100 35 L100 90 L60 97 L20 90 Z"
    stroke="currentColor"
    strokeWidth="1.5"
    fill="none"
    className="text-[var(--library-accent)] opacity-20"
  />
...
function EmptyReservations() {
  return <div className="empty-state py-16 surface rounded-2xl border border-[var(--border-color)]">
    ...
    /* Cases vides */
        ...
        {[0, 1, 2, 3, 4, 5].map((i) => <rect
    key={i}
    x={27 + i % 3 * 22}
    y={65 + Math.floor(i / 3) * 18}
    width="16"
    height="12"
    rx="3"
    stroke="currentColor"
    strokeWidth="1"
    fill="none"
    className="text-[var(--library-accent)] opacity-15"
  />)}
      </svg>
...
export function BorrowsPage({ user }) {
  ...
  const calculateDaysRemaining = (returnDate) => {
    ...
    return Math.ceil((ret.getTime() - today.getTime()) / (1e3 * 60 * 60 * 24));
  };
  const tabClass = (active) => `pb-4 px-6 font-bold transition-all duration-300 focus:outline-none rounded-t-2xl text-sm tap-feedback relative ${active ? "text-accent" : "text-muted hover:text-primary"}`;
  return <div className="min-h-screen bg-library-bg pb-24">
      <Navbar user={user} />
    ...
    className={tabClass(activeTab === "borrows")}
  ...
  >
    ...
    className={tabClass(activeTab === "reservations")}
  ...
  >
    ...
    /* Contenu */
        ...
        <div className="space-y-4">
          {activeTab === "borrows" ? emprunts.length > 0 ? emprunts.map((emprunt) => {
    ...
    return <div key={emprunt.id} className="glass-effect rounded-[2rem] p-6 shadow-card hover:shadow-glow border border-white/10 hover:border-accent/30 transition-all duration-500 animate-flow-in">
                    <div className="flex gap-4">
                      ...
                      <div className="flex-1">
                        <div className="flex items-start justify-between gap-2 mb-2">
                          ...
                          {isOverdue ? <Badge className="bg-red-500/10 text-red-500 border border-red-500/20 flex-shrink-0">En retard</Badge> : isWarning ? <Badge className="bg-amber-500/10 text-amber-500 border border-amber-500/20 flex-shrink-0">Bientôt dû</Badge> : <Badge className="bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 flex-shrink-0">En cours</Badge>}
                        </div>
    ...
    >
                            ...
                            </Button> : <Badge className="surface-weak text-muted border border-[var(--border-color)]">Déjà renouvelé</Badge>}
                        </div>
                      </div>
                    </div>
                  </div>;
  }) : <EmptyBorrows /> : reservations.length > 0 ? reservations.map((reservation) => <div key={reservation.id} className="surface rounded-2xl p-5 shadow-card hover:shadow-card-hover border border-[var(--border-color)] hover:border-[var(--library-accent)]/20 transition-all">
                  <div className="flex gap-4">
                    <div className="hidden sm:block w-16 h-24 rounded-xl overflow-hidden flex-shrink-0 shadow-soft">
                    ...
                    <div className="flex-1">
                      <div className="flex items-start justify-between gap-2 mb-2">
                        ...
                        {reservation.status === "available" ? <Badge className="bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 gap-1">
                            <CheckCircle2 className="w-3 h-3" />Disponible
                          </Badge> : <Badge className="bg-[var(--library-accent)]/10 text-accent border border-[var(--library-accent)]/20 gap-1">
