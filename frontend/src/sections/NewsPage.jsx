...
export function NewsPage({ user, onNewsClick }) {
  ...
  const categories = [
    ...
    { id: "event", label: "Événements", count: news.filter((n) => n.categorie === "event").length },
    { id: "course", label: "Formations", count: news.filter((n) => n.categorie === "course").length },
  ...
  return <div className="min-h-screen bg-library-bg pb-24 transition-colors duration-300">
    ...
    className={`inline-flex items-center gap-3 px-5 py-2.5 rounded-2xl font-bold text-sm transition-all duration-300 border focus:outline-none focus:ring-2 focus:ring-accent/20 ${selectedCategory === cat.id ? "bg-accent text-white border-accent shadow-glow scale-105" : "glass-effect border-white/10 text-primary hover:border-accent/30 hover:text-accent"}`}
  >
                {cat.label}
                <span className={`text-[10px] font-black px-2 py-0.5 rounded-full ${selectedCategory === cat.id ? "bg-white/20" : "bg-white/5 border border-white/10"}`}>
                  {cat.count}
                </span>
              </button>)}
          </div>
    ...
    /* Articles */
        ...
        {sortedNews.length > 0 ? <div className="space-y-5">
            {sortedNews.map((newsItem) => <article
    key={newsItem.id}
  ...
  >
                ...
                {newsItem.image && <div className="md:w-64 w-full aspect-video md:aspect-auto overflow-hidden surface-alt flex-shrink-0">
                    <ApiImage
    src={newsItem.image}
    alt={newsItem.titre}
  ...
  />
                ...
                <div className="flex-1 p-5 md:p-6 flex flex-col justify-between">
                  <div>
                    <div className="flex items-center gap-3 mb-3 flex-wrap">
                      {newsItem.categorie && <Badge className={`text-xs font-semibold px-3 py-1 border ${categoryColors[newsItem.categorie] || categoryColors.general}`}>
                          {categoryLabels[newsItem.categorie] || "Actualité"}
                        </Badge>}
                      {newsItem.misEnAvant && <Badge className="bg-amber-100 text-amber-700 border border-amber-200 dark:bg-amber-900/30 dark:text-amber-300 dark:border-amber-800 text-xs font-semibold px-3 py-1">
                          ✦ À la une
                        </Badge>}
                  ...
                  {newsItem.contenu && <details className="mb-4" onClick={(e) => e.stopPropagation()}>
          ...
          </div> : <div className="surface rounded-2xl p-12 text-center border border-[var(--border-color)]">
