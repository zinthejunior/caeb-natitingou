...
function SkeletonEventCard() {
  return <div className="surface rounded-xl p-3 border border-[var(--border-color)] flex items-center gap-4">
...
function SkeletonNewsCard() {
  return <div className="surface rounded-2xl overflow-hidden border border-[var(--border-color)]">
...
export function HomePage({ user, onNavigate }) {
  ...
  const [iaBooks, setIaBooks] = useState([]);
  useEffect(() => {
  ...
  const recommendedBooks = iaBooks.length > 0 ? iaBooks.slice(0, 4) : user?.genresPreferes && user.genresPreferes.length > 0 ? books.filter((b) => user.genresPreferes?.includes(b.genre || "")).slice(0, 4) : books.slice(0, 4);
  ...
  const upcomingEvents = events.filter((event) => new Date(event.date) >= /* @__PURE__ */ new Date()).slice(0, 3);
  return <div className="min-h-screen bg-library-bg pb-24 transition-colors duration-300">
      <Navbar user={user} />
            ...
            <header className="relative overflow-hidden rounded-[2rem] mesh-gradient-light dark:mesh-gradient-dark border border-[var(--border-color)] shadow-elevated transition-all duration-500 hover:shadow-glow animate-flow-in">
              ...
              <div className="relative p-8 md:p-12 flex flex-col md:flex-row items-start md:items-center justify-between gap-8">
                <div className="flex-1">
                  ...
                  {!user?.estMembre && <div className="mt-8 flex flex-wrap gap-4">
    ...
    /* Actualités */
            ...
            <section className="animate-slide-up">
              ...
              {!dataReady ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                  {[0, 1, 2].map((i) => <SkeletonNewsCard key={i} />)}
                </div> : latestNews.length > 0 ? <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 md:gap-6">
                  {latestNews.map((newsItem) => <div key={newsItem.id} className="list-item-fade">
    ...
    onToggleFavorite={(id) => user.favoris?.includes(id) ? user.favoris = user.favoris.filter((f) => f !== id) : user.favoris = [...user.favoris || [], id]}
  />
                    </div>)}
                </div> : null}
            </section>
    ...
    /* ── BARRE LATÉRALE ── */
          ...
          <div className="lg:col-span-4 space-y-8">
            <section className="animate-slide-up" style={{ animationDelay: "400ms" }}>
              ...
              {!dataReady ? <div className="space-y-3">
                  {[0, 1, 2].map((i) => <SkeletonEventCard key={i} />)}
                </div> : <div className="space-y-3">
                  {upcomingEvents.map((event) => <EventCard key={event.id} event={event} user={user} onClick={() => onNavigate("event-detail", { eventId: event.id })} />)}
                </div>}
            </section>
...
function SectionHeader({ title, icon: Icon, onSeeAll }) {
  return <div className="flex items-center justify-between mb-5 px-1">
...
function BookCard({ book, user, onClick, onToggleFavorite }) {
  ...
  >
    ...
    /* Overlay de verrouillage */
        ...
        {!user?.estMembre && book.exemplaires <= 0 && <div className="absolute inset-0 surface/80 backdrop-blur-[2px] flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
  ...
  />)}
          ...
          {!user?.estMembre && <span className="text-[10px] font-bold text-accent bg-[var(--library-accent)]/10 px-2 py-0.5 rounded-full">PREMIUM</span>}
        </div>
      </div>
    </div>;
}
...
function NewsCard({ news, onClick }) {
  ...
  return <button onClick={onClick} className="w-full text-left group flex flex-col h-full tap-feedback">
      <div className={`surface rounded-2xl overflow-hidden shadow-card hover:shadow-card-hover border border-[var(--border-color)] hover:border-[var(--library-accent)]/20 transition-all duration-300 flex flex-col h-full`}>
        {news.image && <div className="relative w-full aspect-video overflow-hidden bg-[var(--library-surface-alt)]">
            <ApiImage
    src={news.image}
    alt={news.titre}
    className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500"
