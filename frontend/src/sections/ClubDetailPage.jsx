...
export function ClubDetailPage({ user }) {
  ...
  if (isClubLoading || isEventsLoading) return <div className="min-h-screen bg-library-bg flex items-center justify-center">
  ...
  >
    ...
    /* External link */
            ...
            {club.lienExterne && <div className="surface rounded-2xl p-6 border border-[var(--border-color)] shadow-soft">
  ...
  >
    ...
    /* Courses */
            ...
            {club.cours && club.cours.length > 0 && <div className="surface rounded-2xl p-6 border border-[var(--border-color)] shadow-soft">
                ...
                <div className="space-y-3">
                  {club.cours.map((course) => <div key={course.id} className="p-3 rounded-xl bg-[var(--library-surface-alt)] border border-[var(--border-color)]">
    ...
    /* Recent activity */
            ...
            {club.activiteRecente && club.activiteRecente.length > 0 && <div className="surface rounded-2xl p-6 border border-[var(--border-color)] shadow-soft">
                <h2 className="font-display text-xl font-semibold text-[var(--library-text)] mb-3 flex items-center gap-2">
                  <Activity className="w-5 h-5 text-[var(--library-accent)]" />
                  Activité des membres
                </h2>
                <div className="space-y-2">
                  {club.activiteRecente.map((act) => <div key={act.id} className="flex items-start gap-3 p-3 bg-[var(--library-surface-alt)] rounded-xl border border-[var(--border-color)]">
    ...
    /* Club events */
            ...
            {clubEvents.length > 0 && <div>
                <h2 className="font-display text-xl font-semibold text-[var(--library-text)] mb-4">
                  Prochains événements du club
                </h2>
                <div className="grid sm:grid-cols-2 gap-4">
                  {clubEvents.map((event) => <EventCard key={event.id} event={event} onParticipate={handleParticipate} />)}
                </div>
              </div>}
          </div>
    ...
    /* Manager */
            ...
            {club.responsable && <div className="surface rounded-2xl p-6 border border-[var(--border-color)] shadow-soft">
  ...
  >
    ...
    /* Next meetings */
            ...
            {club.prochainesReunions && club.prochainesReunions.length > 0 && <div className="surface rounded-2xl p-6 border border-[var(--border-color)] shadow-soft">
    ...
    /* Stats card */
        ...
        {showContactForm && <ClubContactForm
...
function EventCard({ event, onParticipate }) {
  ...
  return <div className="surface rounded-xl p-4 border border-[var(--border-color)] hover:border-[var(--library-accent)]/30 hover:shadow-soft transition-all">
    ...
    className={`w-full ${event.participe ? "border-[var(--border-color)] text-[var(--library-muted)]" : "bg-[var(--library-accent)] text-[var(--library-on-accent)] hover:opacity-90 sheen"}`}
  >
        {event.participe ? <><Check className="w-4 h-4 mr-1" />Inscrit</> : "Je participe"}
      </Button>
    </div>;
}
