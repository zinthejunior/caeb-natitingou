...
class SearchService:
    ...
    async def _search_serpapi(cls, query: str, limit: int) -> List[Dict[str, str]]:
        ...
        if not SERPAPI_API_KEY:
            logger.warning("[SerpAPI] Pas de clé SERPAPI_API_KEY configurée")
            return []
        try:
            logger.debug(f"[SerpAPI] Requête : {query}")
            async with httpx.AsyncClient(timeout=8.0) as client:
                resp = await client.get(
                    "https://serpapi.com/search.json",
                    params={"q": query, "api_key": SERPAPI_API_KEY, "num": limit, "hl": "fr"},
                )
                logger.debug(f"[SerpAPI] Status : {resp.status_code}")
                if resp.status_code == 200:
                    ...
                    logger.info(f"[SerpAPI] ✅ {len(results)} résultats récupérés")
                    return results
                else:
                    logger.error(f"[SerpAPI] Erreur HTTP {resp.status_code} : {resp.text[:200]}")
        except Exception as e:
            logger.error(f"[SerpAPI] Exception : {type(e).__name__} - {str(e)}")
        return []
    ...
    async def search_web(cls, query: str, limit: int = 5, fetch_page_content: bool = False) -> List[Dict[str, str]]:
        ...
        logger.info(f"[SearchService] Début recherche web pour : '{query[:60]}...'")
        results: List[Dict[str, str]] = []

        # 1. SerpAPI
        logger.info(f"[SearchService] Tentative SerpAPI (clé configurée: {bool(SERPAPI_API_KEY)})")
        results = await cls._search_serpapi(query, limit)
        if results:
            logger.info(f"[SearchService] SerpAPI réussi → {len(results)} résultats")
            return (await cls._attach_page_content(results, max_pages=min(limit, 3))
                    if fetch_page_content else results)

        # 2. Bing
        logger.info(f"[SearchService] Tentative Bing (clé configurée: {bool(BING_SEARCH_API_KEY)})")
        ...
        logger.info("[SearchService] Tentative SearXNG...")
        results = await cls._search_searxng(query, limit)
        if results:
            logger.info(f"[SearchService] SearXNG réussi → {len(results)} résultats")
            return (await cls._attach_page_content(results, max_pages=min(limit, 3))
                    if fetch_page_content else results)

        # 4. DuckDuckGo Instant Answers
        logger.info("[SearchService] Tentative DuckDuckGo JSON...")
        ...
        logger.info("[SearchService] Tentative Wikipedia FR...")
        results = await cls._search_wikipedia(query, limit)
        if results:
            logger.info(f"[SearchService] Wikipedia réussi → {len(results)} résultats")
            return results
