...
from fastapi_kossi.core.security import get_current_auth
...
async def chat_stream(request: ChatRequest, req: Request):
    ...
    async def generate_sse():
        ...
        try:
            ...
            is_scholar_query = intent == "scholar"
            ...
            if is_web_query:
                ...
                try:
                    ...
                    for book in matched_books:
                        sources.append({
                            ...
                            "url": f"http://localhost:5173/catalogue?q={book.get('titre') or book.get('title')}",
            ...
            elif is_scholar_query:
                # Pour les questions académiques, chercher d'abord dans le catalogue
                try:
                    books = DatabaseService.get_all_books()
                    if books:
                        top_titles = await SearchService.hybrid_search_books(books, request.message, limit=3)
                        title_set = set(top_titles)
                        for book in books:
                            titre = book.get("titre") or book.get("title") or ""
                            if titre in title_set:
                ...
                # Si pas de livres pertinents, faire une recherche web
                if not matched_books:
                    logger.info("[/chat/stream] Aucun livre pertinent trouvé pour scholar query. Lancement de la recherche web.")
                    web_results = await SearchService.search_web(request.message, limit=5, fetch_page_content=True)
                    sources = web_results
            ...
            if is_web_query and web_results:
                ...
                messages.append({
                    ...
                    "content": (
                        ...
                        "NE PAS inclure les URLs ou une section 'Sources consultées' à la fin du message - "
                        "les sources seront affichées séparément dans une section dédiée."
                    ),
                })
            elif is_web_query:
                messages.append({
            ...
            elif is_scholar_query and web_results:
                # Pour scholar queries avec résultats web (quand aucun livre du catalogue)
                web_context_lines = [
                ...
                messages.append({
                    "role": "system",
                    "content": (
                        "Tu es l'Agent Érudit de la Bibliothèque CAEB de Natitingou. "
                        "Utilise les résultats web ci-dessus pour répondre de manière précise et factuelle. "
                        "Cite tes sources en utilisant les numéros entre crochets comme [1], [2], etc. "
                        "NE PAS inclure les URLs ou une section 'Sources consultées' à la fin du message - "
                        "les sources seront affichées séparément dans une section dédiée."
                    ),
                })
            ...
            full_response = ""
            reply_generator = orchestrator.process_stream(
                messages=messages,
                message=request.message,
                history=history_dicts,
                user_profile=user_data,
            ...
            if inspect.isawaitable(reply_generator):
                reply_generator = await reply_generator
...
async def vectorize_book(request: VectorizeRequest, req: Request):
    ...
    try:
        ...
        # Sauvegarde dans la BD locale/partagée
