...
class ScholarAgent:
    ...
    async def execute(self, query: str, context: dict) -> str:
        ...
        # Ajouter les résultats web s'ils existent
        if web_results:
            sources.extend(web_results)
        elif not rag_books:
            # Si aucun livre du catalogue n'est pertinent, créer une source informative
            sources.append({
                "title": "🌐 Ressources recommandées",
                "url": "http://localhost:5173/",
                "snippet": "Pour une étude plus approfondie sur ce sujet, consultez les archives spécialisées ou contactez nos bibliothécaires."
            })
