...
from fastapi_kossi.services.intent_service import detect_intent_llm
...
async def _fetch_backend_data(endpoint: str) -> List[Dict[str, Any]]:
    """Récupère des données depuis la BD PostgreSQL locale/partagée."""
    try:
        if endpoint == "clubs":
...
async def _fetch_all_books() -> List[Dict[str, Any]]:
    """Récupère l'intégralité du catalogue de livres depuis la BD."""
    try:
...
class KossiOrchestrator:
    ...
    async def process(
        ...
        **kwargs,
        ...
        # ── 1. Détection d'intention (hybride LLM + regex) ────────────────────
        #
        # Règle :
        #   • 1er message d'une session (history vide)  → LLM gratuit (précision max)
        #   • Messages suivants                          → regex rapide d'abord
        #       - si regex trouve quelque chose          → on l'utilise
        #       - si regex ne trouve rien (retourne []   → LLM en fallback
        #         ou uniquement ["scholar"] qui est le
        #         catch-all par défaut du regex)
        #
        # Dans tous les cas, si le LLM est indisponible (pas de clé API,
        # timeout, erreur réseau), on retombe silencieusement sur le regex.
        ...
        if is_first_message:
            # Premier message : LLM en priorité
            logger.info("[Orchestrateur] 1er message → classification LLM")
            llm_intent = await detect_intent_llm(query)
            if llm_intent:
                candidate_intents = [llm_intent]
            else:
                # LLM indisponible → regex de secours
                logger.info("[Orchestrateur] LLM indisponible → regex de secours")
                candidate_intents = _detect_intents(query)
        else:
            # Messages suivants : regex d'abord
            candidate_intents = _detect_intents(query)
            regex_is_ambiguous = (
                not candidate_intents
                or candidate_intents == ["scholar"]  # catch-all regex = souvent faux positif
            )
            if regex_is_ambiguous:
                logger.info("[Orchestrateur] Regex ambigu → fallback LLM")
                llm_intent = await detect_intent_llm(query)
                if llm_intent:
                    candidate_intents = [llm_intent]
                # sinon on garde candidate_intents tel quel (scholar ou [])
        ...
        logger.info(f"[Orchestrateur] Méthode : {'LLM' if llm_intent else 'regex'}")
        ...
        if "web" in candidate_intents or intent in ("scholar", "general"):
            logger.info(f"[Orchestrateur] Lancement de la recherche web pour : '{query[:50]}...'")
            web_results = await SearchService.search_web(query, limit=5, fetch_page_content=True)
            logger.info(f"[Orchestrateur] Résultats web reçus : {len(web_results)} résultats")
        else:
            logger.info(f"[Orchestrateur] Recherche web non pertinente (intent={intent}, web_in_candidates={'web' in candidate_intents})")
